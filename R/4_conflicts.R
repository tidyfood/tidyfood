#' Conflicts between the tidyfood and other packages
#'
#' This function lists all the conflicts between packages in the tidyfood
...
tidyfood_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", tidyfood_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  structure(conflict_funs, class = "tidyfood_conflicts")
}

tidyfood_conflict_message <- function(x) {
  if (length(x) == 0)
    return("")

  header <- cli::rule(left = crayon::bold("Conflicts"),
                      right = "tidyfood_conflicts()")
...
#' @export
print.tidyfood_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(tidyfood_conflict_message(x))
}
